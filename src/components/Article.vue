<template>
	<div>
		...
		<div class="content">
			<div class="header clear"><h2></h2></div>
		    <div class="cont">
		    	<h3>{{article.title}}</h3>
		        <div class="time"><p>{{article.time}} <span><img src="../assets/img/zan.png" alt=""/></span></p></div>
...
<script>
	export default {
		...
		methods:{
			fetchData(id){
				var that = this;
				this.$http.get('../../data/article.data').then(function(res){
					that.article = res.data[id];
				});
			}
		}
	}
